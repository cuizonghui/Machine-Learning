...
class DecisionTreeClassification:
    def __init__(self, epsilon=0.01, is_information_gain=True):
        self._epsilon = epsilon
        self._tree = {}
        self._is_information_gain = is_information_gain
    ...
    def fit(self, features, labels, features_name):
        ...
        best_feature_name, best_feature_index, best_feature_info_gain = self._get_best_feature(
            features, labels, features_name)

        # 信息增益小于阈值epsilon则设置为单节点树，并讲实例中最大类别Ｃｋ设置为节点类别，返回Ｔ
        if best_feature_info_gain < self._epsilon:
            labels_frequency_dict = dict(Counter(labels))
            return Node(root=True, label=max(labels_frequency_dict, key=labels_frequency_dict.get))
    ...
    def _get_best_feature(self, features, labels, features_name):
        ...
        for index in range(len(features[0])):
            ...
            information_gain_dict[features_name[index]] = self._information_gain_or_information_gain_ratio(
                information_entropy, condition_entropy_value, self._is_information_gain)
        best_feature_name = max(information_gain_dict, key=information_gain_dict.get)
        best_feature_index = np.where(features_name == best_feature_name)[0][0]
        return best_feature_name, best_feature_index, information_gain_dict[best_feature_name]

    def _information_gain_or_information_gain_ratio(self, information_entropy, condition_entropy, is_information_gain):
        """
        信息增益或者信息增益比
    ...
    def _calculate_condition_entropy(self, feature, labels):
        ...
        for key in feature_labels_dict.keys():
            condition_entropy_value += probability_dict[key] * self._calculate_information_entropy(
                feature_labels_dict[key])
...
def main():
    ...
    decision_tree_classification = DecisionTreeClassification(epsilon=0.01, is_information_gain=False)
